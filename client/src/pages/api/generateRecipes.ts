...
if (!apiKey) {
  console.error("The OpenAI API key is missing or empty. Please check your environment variables.");
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: "The OpenAI API key is missing or empty. Please check your environment variables." });
  }
